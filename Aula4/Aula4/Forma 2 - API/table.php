...
<body>
    <?php
        ...
        $arrayFiltrado = array_filter($pessoas, function ($pessoa){
            ...
            /*if( ((stripos($pessoa["nome"], $nome) > -1) || empty($nome)) 
            && ((stripos($pessoa["cidade"], $cidade) > -1) || (empty($cidade))) 
            && (($idadeForm == $idade ) || (empty($idadeForm))) 
            && (($estado == $pessoa['estado']) || (empty($estado))) ){
                return true;
            }*/

            if((empty($nome) || (stripos($pessoa["nome"], $nome) > -1)) && ((empty($idadeForm) || ($idadeForm == $idade))) && (empty($cidade) || (stripos($pessoa['cidade'], $cidade) > -1) ) &&(empty($estado) || ($estado == $pessoa['estado']))){
            return 1;
            }
            ...
            echo "<table border= '1px'>";
                ...
                foreach($array as $pessoa){
                    echo"<tr>";
                        ...
                        echo"<td>".$pessoa['dtNascimento']."</td>";
